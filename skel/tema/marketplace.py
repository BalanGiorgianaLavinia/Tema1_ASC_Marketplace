...
class Marketplace:
    ...
    def __init__(self, queue_size_per_producer):
        ...
        #append an empty list in order to start from index 1
    ...
    def publish(self, producer_id, product):
        ...
        #add product in list if there is still space
    ...
    def add_to_cart(self, cart_id, product):
        ...
        :returns True or False. If the caller receives False, it should wait and then try again
        """
        #search for product in the dictionary with producers and products
        for producer_id in range(1, self.producers_id + 1):
            if product in self.producers_queues[producer_id]:
                #remove it from dictionary and add it in cart
    ...
    def remove_from_cart(self, cart_id, product):
        ...
        """
        #search for product in cart and take the producer_id
        for pair in self.list_of_carts[cart_id]:
            if pair[0] == product:
                self.producers_queues[pair[1]].append(product)
                self.list_of_carts[cart_id].remove(pair)
                return
